...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed):
        ...
        language_reference_points = reference_points[:, :, self.num_points//2, :].unsqueeze(dim=2)
        reference_points = torch.cat([reference_points, language_reference_points], dim=2)
...
class DeformableCompositeTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        attn_mask = torch.zeros((tgt.shape[2], tgt.shape[2]), dtype=torch.bool, device=tgt.device)
        attn_mask[:, -1] = True
        ...
        if len(reference_points.shape) == 4:
            reference_points_loc = reference_points[:, :, None, :, :].repeat(1, 1, tgt.shape[2], 1, 1)
        else:
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.ctrl_point_coord is not None:
                ...
                new_reference_points = tmp + inverse_sigmoid(reference_points[:, :, :-1, :])
                new_reference_points = new_reference_points.sigmoid()
                reference_points = new_reference_points.detach()
                language_reference_points = reference_points[:, :, reference_points.shape[2]//2, :].unsqueeze(dim=2)
                reference_points = torch.cat([reference_points, language_reference_points], dim=2)
